class Scene2Players extends Phaser.Scene {
    ...
    constructor ()
    {
        ...
        this.numberErrorsPlayer1 = 0;
        this.numberErrorsPlayer2 = 0;
    }
    ...
    create ()
    {
        ...
        this.emojiSad = this.add.image(this.x+130, this.y-250,'sad');
        ...
        if(this.countClicks == 2){
            ...
            if(!this.equalSelectedImages()){
                if(this.player1text.visible){
                    setTimeout(() => {
                        this.numberErrorsPlayer1++;
                        this.flipImages();
                        this.showChangeTurnMessage("Player2");
                        this.player1text.visible=false;
                        this.player2text.visible=true;
                    ...
                    setTimeout(() => {
                        this.numberErrorsPlayer2++;;
                        this.flipImages();
                        this.showChangeTurnMessage("Player1");
                        this.player1text.visible=true;
                        this.player2text.visible=false;
                        this.updateClickedImages();
                        this.numCorrects = 0;
                        this.freeze = false;
                     }, 1500);  
                ...
                if(this.player1text.visible){
                    ...
                    setTimeout(() => {
                    this.numCorrectsPlayer2++; 
                    this.player2text.setText('Player2: '+ this.numCorrectsPlayer2);
                }, 1700); 
                }
                if(this.numCorrectsPlayer1 + this.numCorrectsPlayer2 == 8){
                    if(this.numCorrectsPlayer1 > this.numCorrectsPlayer2 ){
                        setTimeout(() => {
                        ...
                        this.player1text.visible = false;
                        this.player2text.visible = false;
                        finishTracking(window.location.href, 'Matching Pairs Game 2 Players', 7,this.numberErrorsPlayer1 + this.numberErrorsPlayer2);
                    }, 2700); 
                    }else{
                        setTimeout(() => {
                        this.winnerText.setText("Congratulations Player2!!" );
                        this.winnerText.visible=true;
                        this.clickButton.visible=true;
                        this.player1text.visible = false;
                        this.player2text.visible = false;
    ...
    showChangeTurnMessage(player){
        this.emojiSad.setVisible(true);
        this.changeTurnText.setText(player + "'s turn!!");
        this.changeTurnText.visible=true;
        setTimeout(() => {
            this.changeTurnText.visible=false;
            this.emojiSad.setVisible(false);
        }, 1500);
    }
    ...
    createText(){
        ...
        this.player2text = this.add.text(this.x-350, this.y-270);
        this.player2text.setStyle({ fontFamily: 'myFont', fontSize:30, fill: '#FFFFFF'});
        this.player2text.setText('Player2: '+ this.numCorrectsPlayer2);
        this.player2text.visible=false;

        this.changeTurnText = this.add.text(this.x-110, this.y-270);
        this.changeTurnText.setStyle({ fontFamily: 'myFont', fontSize:30, fill: '#ffff00'});
        this.changeTurnText.setText('Oops you have failed !!!' + "'s turn!!");
        this.changeTurnText.visible=false;
